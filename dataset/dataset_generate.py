...
with open('QnA_dataset.jsonl', 'w') as f:
    # For each concept in DataFrame
    for index, row in df.iterrows():
        concept = row['concept']
        context = row['context']
        prerequisites = row['prerequisites'].split(", ")

        # Shuffle the prerequisites
        random.shuffle(prerequisites)
        
        # Chunk prerequisites into groups of 1 or 2
        prereq_chunks = list(chunks(prerequisites, random.choice([2, 3])))

        # Generate questions per prerequisite chunk
        for prereq_chunk in prereq_chunks:
            ...
            prereq_to_use = ", ".join(prereq_chunk)
